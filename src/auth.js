(function () {
  ...
  const SOCIAL_CONFIG = {
    ...
    microsoftTenant:
      String(window.MEMORY_VAULT_MICROSOFT_TENANT || "common").trim() ||
      "common",
  };
  ...
  function resolveApiBaseUrl() {
    ...
    const raw = String(
      window.MEMORY_VAULT_API_URL ||
        (window.Capacitor ? liveFallbackApi : sameOriginApi),
    ).trim();
  ...
  function getSocialProviderAvailability(provider) {
    if (provider === "google")
      return {
        enabled: Boolean(SOCIAL_CONFIG.googleClientId),
        reason: "Google sign-in is not configured.",
      };
    if (provider === "facebook")
      return {
        enabled: Boolean(SOCIAL_CONFIG.facebookAppId),
        reason: "Facebook sign-in is not configured.",
      };
    if (provider === "microsoft")
      return {
        enabled: Boolean(SOCIAL_CONFIG.microsoftClientId),
        reason: "Microsoft sign-in is not configured.",
      };
  ...
  function updateSocialButtons() {
    const buttons = [
      ...document.querySelectorAll(".social-btn[data-provider]"),
    ];
    let enabled = 0;
    ...
    return new Promise((resolve, reject) => {
      ...
      const pollTimer = setInterval(() => {
        ...
        try {
          ...
          const state =
            queryParams.get("state") || hashParams.get("state") || "";
          const accessToken =
  ...
  async function socialAuthSignIn(provider) {
    ...
    try {
      ...
      const redirectUri =
        "https://memory-vault-coral-seven.vercel.app/auth.html";
      ...
      if (isMobile && window.Capacitor.Plugins.Browser) {
        ...
        if (provider === "google") {
          ...
          authUrl = `https://www.facebook.com/v19.0/dialog/oauth?client_id=${encodeURIComponent(
            SOCIAL_CONFIG.facebookAppId,
          )}&redirect_uri=${encodeURIComponent(
            redirectUri,
          )}&state=${state}&scope=email,public_profile&response_type=token`;
      ...
      if (provider === "google") {
          ...
          window.FB.login(
            (response) => {
              if (!response?.authResponse?.accessToken)
                return reject(new Error("Facebook sign-in was cancelled."));
        ...
        const pkce = await createPkcePair();
        const state = `microsoft_${Date.now()}_${Math.random()
          .toString(36)
          .slice(2, 8)}`;
  ...
  async function submitAuth(event) {
    ...
    let username = String(els.username.value || "")
      .trim()
      .toLowerCase();

    if (!identifier)
      return showMsg("Please enter your email or username.", "error");
    ...
    try {
      ...
      if (authMode === "signup") {
        ...
        if (!(/[A-Za-z]/.test(password) && /\d/.test(password)))
          return showMsg("Password must include letters and numbers.", "error");
  ...
  document.querySelectorAll(".social-btn[data-provider]").forEach((btn) => {
    btn.addEventListener("click", () =>
      socialAuthSignIn(
        String(btn.getAttribute("data-provider") || "").toLowerCase(),
      ),
    );
  });
    ...
    if (code || token) {
      ...
      const provider =
        ...
        (state.startsWith("google_")
          ? "google"
          : state.startsWith("facebook_")
          ? "facebook"
          : state.startsWith("microsoft_")
          ? "microsoft"
          : "");
  ...
  const pathModeMatch = window.location.pathname.match(
    /\/auth\/(signin|signup)\/?$/i,
  );
  const modeFromPath = String(
    (pathModeMatch && pathModeMatch[1]) || "",
  ).toLowerCase();
  const modeFromQuery = String(
    new URLSearchParams(window.location.search).get("mode") || "",
  ).toLowerCase();
  const initialMode =
    modeFromPath || (modeFromQuery === "signup" ? "signup" : "signin");
  ...
  if (resetToken && resetEmail) {
    showMsg(
      "Password reset link received. Use the form below to change password.",
      "",
    );
  }
  ...
  if (
    ...
    console.log(
      "Detected social auth return in browser. Redirecting to app...",
    );
    ...
    setTimeout(() => {
      showMsg(
        "Login successful! <br><br> <button onclick=\"window.location.assign('" +
          deepLink +
          '\')" style="padding: 12px 24px; background: #6d36ff; color: white; border: none; border-radius: 8px;">Back to App</button>',
        "ok",
      );
    }, 1500);
  }
