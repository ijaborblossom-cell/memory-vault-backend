...
const defaultConfig = {
  ...
  hero_subtitle:
    "Keep class notes, hard lessons, and personal reflections in a space that feels like your own journal, not a machine.",
...
let personalUnlockExpiresAt = Number(
  sessionStorage.getItem("personal_unlock_expires_at") || 0,
);
...
const AUTH_ROUTE_MATCH = window.location.pathname.match(
  /^\/auth(?:\/(signin|signup))?\/?$/i,
);
...
const SOCIAL_CONFIG = {
  ...
  microsoftClientId: String(
    window.MEMORY_VAULT_MICROSOFT_CLIENT_ID || "",
  ).trim(),
  microsoftTenant:
    String(window.MEMORY_VAULT_MICROSOFT_TENANT || "common").trim() || "common",
};
...
async function checkServerHealth() {
  try {
    const response = await fetch(`${API_BASE_URL}/health`, {
      method: "GET",
      signal: buildRequestSignal(15000),
    });
...
function updateConnectionStatus(connected, customMessage = null) {
  ...
  if (connected) {
    ...
    setTimeout(() => {
      if (serverConnected) statusEl.style.display = "none";
    }, 2000);
  } else {
...
async function processRequestQueue() {
  ...
  for (const request of queue) {
    ...
    if (Date.now() - request.timestamp > 300000) {
      ...
      if (request.callback)
        request.callback({ success: false, message: "Request expired" });
      continue;
    }
    ...
    try {
      ...
      if (request.callback)
        request.callback({ success: false, message: error.message });
    }
  }
}
...
function initializeAppUi() {
  ...
  if (IS_AUTH_ROUTE) {
    const routeMode = String(
      (AUTH_ROUTE_MATCH && AUTH_ROUTE_MATCH[1]) || "",
    ).toLowerCase();
  ...
  if (streakNote) {
    if (streak >= 7) {
      streakNote.textContent =
        "Strong momentum. Keep protecting your study streak.";
    } else if (streak >= 3) {
...
function toggleNavMenu(event, isMobile = false) {
  ...
  const menu = document.getElementById(
    isMobile ? "mobile-nav-dropdown-menu" : "nav-dropdown-menu",
  );
  const btn = document.getElementById(
    isMobile ? "mobile-nav-menu-btn" : "nav-menu-btn",
  );
  if (!menu || !btn) return;
...
async function loadMemoriesFromBackend() {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      console.log(
        `Loaded ${memories.length} memories from backend on startup.`,
      );
    }
  } catch (error) {
...
async function onConfigChange(config) {
  ...
  document.getElementById("app-tagline").textContent =
    config.app_tagline || defaultConfig.app_tagline;
  document.getElementById("hero-title").textContent =
    config.hero_title || defaultConfig.hero_title;
  document.getElementById("hero-subtitle").textContent =
    config.hero_subtitle || defaultConfig.hero_subtitle;
  document.getElementById("personal-vault-name").textContent =
    config.personal_vault_name || defaultConfig.personal_vault_name;
  document.getElementById("learning-vault-name").textContent =
    config.learning_vault_name || defaultConfig.learning_vault_name;
  document.getElementById("cultural-vault-name").textContent =
    config.cultural_vault_name || defaultConfig.cultural_vault_name;
  document.getElementById("future-vault-name").textContent =
    config.future_vault_name || defaultConfig.future_vault_name;
}
    ...
    if (window.elementSdk) {
      await window.elementSdk.init({
        ...
        mapToCapabilities: () => ({
          ...
          fontEditable: {
            get: () =>
              window.elementSdk?.config?.font_family ||
...
async function apiCall(
  ...
  const options = {
    ...
    headers: {
      ...
      ...(includePersonalToken && {
        "x-personal-unlock-token": personalUnlockToken,
      }),
  ...
  try {
    const response = await fetch(`${API_BASE_URL}${endpoint}`, {
      ...options,
      signal: buildRequestSignal(20000), // 20 second timeout
    });
    ...
    const errorMessage = String(error?.message || error || "Unknown API error");
    const errorName = String(error?.name || "");
    console.error("API Error:", errorMessage);

    // Network error - queue the request
    if (
      /failed to fetch|networkerror|network request failed|load failed|internet disconnected|timeout|abort/i.test(
        errorMessage,
      ) ||
      errorName === "TypeError" ||
      ...
      if (normalizedMethod !== "GET") {
        return {
          success: false,
          message:
            "Network issue: could not reach server. Please retry when connected.",
        };
      }
...
async function createMemory(title, content, is_important, vault_type) {
  try {
    const normalizedVaultType =
      normalizeVaultKey(vault_type) ||
      String(vault_type || "")
        .trim()
        .toLowerCase();
    ...
    const emergencyLocalSave = (fallbackRecord) => {
      try {
        const storageKey = localStorage.getItem("user_email")
          ? `memory_vault_data_${localStorage.getItem("user_email")}`
          : "memory_vault_data_guest";
      ...
      return {
        ...
        isOk: true,
        data: fallbackRecord,
        message: "Saved locally (emergency mode).",
      };
    };
    ...
    const saveLocally = async () => {
      ...
      return emergencyLocalSave(fallbackRecord);
    };
...
function openAddForm(vault) {
  ...
  const titles = {
    learning: "Add Academic Memory",
    cultural: "Add Cultural Memory",
    future: "Add Future Wisdom",
    personal: "Write Diary Entry",
  };
...
function memorySignature(memory) {
  return [
    (memory.title || "").trim(),
    (memory.content || "").trim(),
    normalizeVaultKey(memory.vault_type) ||
      String(memory.vault_type || "")
        .trim()
        .toLowerCase(),
...
async function syncLocalMemoriesToBackend(email) {
  ...
  const backendMemories =
    backendResult.success && Array.isArray(backendResult.data)
      ? backendResult.data
      : [];
  const backendSet = new Set(backendMemories.map(memorySignature));
  const toImport = localMemories.filter(
    (m) => !backendSet.has(memorySignature(m)),
  );
...
function storePersonalUnlockState(token, expiresAt) {
  ...
  if (personalUnlockToken && personalUnlockExpiresAt) {
    ...
    sessionStorage.setItem(
      "personal_unlock_expires_at",
      String(personalUnlockExpiresAt),
    );
  } else {
    sessionStorage.removeItem("personal_unlock_token");
    sessionStorage.removeItem("personal_unlock_expires_at");
...
async function submitPin() {
  ...
  if (pinAttempt.length < 4) {
    msgDiv.innerHTML =
      '<div class="message error">PIN must be at least 4 digits</div>';
    return;
  }
  ...
  try {
    if (authToken) {
      const status = await apiCall("/personal/pin/status", "GET");
      if (!status.success) {
        msgDiv.innerHTML = `<div class="message error">${escapeHtml(
          status.message || "Could not check PIN status",
        )}</div>`;
        currentPin = "";
        updatePinDisplay();
        return;
      ...
      if (!status.configured) {
        const setupResult = await apiCall("/personal/pin/setup", "POST", {
          pin: pinAttempt,
        });
        if (!setupResult.success) {
          msgDiv.innerHTML = `<div class="message error">${escapeHtml(
            setupResult.message || "PIN setup failed",
          )}</div>`;
          currentPin = "";
          updatePinDisplay();
          return;
      ...
      const verifyResult = await apiCall("/personal/pin/verify", "POST", {
        pin: pinAttempt,
      });
      if (!verifyResult.success) {
        msgDiv.innerHTML = `<div class="message error">${escapeHtml(
          verifyResult.message || "Incorrect PIN",
        )}</div>`;
        currentPin = "";
        updatePinDisplay();
        return;
      ...
      storePersonalUnlockState(
        verifyResult.unlockToken,
        verifyResult.expiresAt,
      );
      currentPin = "";
      updatePinDisplay();
      msgDiv.innerHTML =
        '<div class="message success">Personal vault unlocked.</div>';
      unlockDiary();
      return;
    }
...
async function resetPin() {
  if (authToken) {
    const accountPassword = prompt(
      "Enter your account password to reset your personal vault PIN:",
    );
    ...
    if (!newPin || !/^\d{4,6}$/.test(newPin)) {
      document.getElementById("pin-message").innerHTML =
        '<div class="message error">New PIN must be 4-6 digits.</div>';
      return;
    }

    const result = await apiCall("/personal/pin/reset", "POST", {
    ...
    if (!result.success) {
      document.getElementById(
        "pin-message",
      ).innerHTML = `<div class="message error">${escapeHtml(
        result.message || "PIN reset failed",
      )}</div>`;
      return;
    }
    ...
    updatePinDisplay();
    document.getElementById("pin-message").innerHTML =
      '<div class="message success">PIN reset and personal vault unlocked.</div>';
    unlockDiary();
    return;
  }

  const shouldReset = confirm(
    "Reset your PIN? All diary entries will remain safe. You'll just create a new PIN to access them.",
  );
  ...
  if (shouldReset) {
    ...
    document.getElementById("pin-message").innerHTML =
      '<div class="message success">PIN cleared. Create a new 4-6 digit PIN.</div>';
  }
}
...
document.getElementById("memory-form").addEventListener("submit", async (e) => {
  ...
  if (memories.length >= 1000000) {
    showMessage("Storage limit reached (1,000,000 memories).", "error");
    return;
  }
  ...
  try {
    ...
    // Emergency local fallback so users do not lose input on unexpected runtime errors.
    try {
      const fallbackRecord = {
        title: memory.title,
      ...
      const storageKey = (() => {
        try {
          const email = localStorage.getItem("user_email");
          return email
            ? `memory_vault_data_${email}`
            : "memory_vault_data_guest";
        } catch {
          return "memory_vault_data_guest";
        }
      })();
      const existing = JSON.parse(localStorage.getItem(storageKey) || "[]");
      existing.push(fallbackRecord);
      localStorage.setItem(storageKey, JSON.stringify(existing));
      memories.push(fallbackRecord);
      invalidateTrendingCache();
      updateCounts();
      renderCurrentView();
      showMessage(
        "Memory saved locally. Backend will sync when connection is back.",
        "success",
      );
      document.getElementById("memory-form").reset();
      setTimeout(() => {
      ...
      console.error("Emergency storage failed:", err);
      const isQuotaError =
        err.name === "QuotaExceededError" ||
        err.code === 22 ||
        err.message.includes("quota");
      const detail = isQuotaError ? " (Storage is full)" : "";
      showMessage(`Failed to save memory${detail}. Please try again.`, "error");
      btn.disabled = false;
      text.textContent = "Save Memory";
    }
  }
...
async function deleteMemory(id) {
  try {
    ...
    if (memory.__backendId && String(memory.__backendId) === String(id)) {
      ...
      if (!(result.success || result.isOk)) {
        showMessage(
          `Failed to delete memory: ${result.message || "Unknown error"}`,
          "error",
        );
        return;
      }
    ...
    if (authToken) {
      ...
      if (!(result.success || result.isOk)) {
        ...
        const notFoundRemotely = String(result.message || "")
          .toLowerCase()
          .includes("not found");
        const networkFailure = /network|fetch|timeout|abort/i.test(
          String(result.message || ""),
        );
        if (
          (localOnlyId && String(localOnlyId) === String(id)) ||
          notFoundRemotely ||
          networkFailure
        ) {
          ...
          if (localDelete.success || localDelete.isOk) {
            result = {
              ...
              message: networkFailure
                ? "Memory deleted locally (will sync when online)."
                : "Memory deleted (local fallback)",
            };
          }
        }
      }
...
function updateCounts() {
  const counts = {
    learning: memories.filter(
      (m) => normalizeVaultKey(m.vault_type) === "learning",
    ).length,
    cultural: memories.filter(
      (m) => normalizeVaultKey(m.vault_type) === "cultural",
    ).length,
    future: memories.filter((m) => normalizeVaultKey(m.vault_type) === "future")
      .length,
  };
...
function renderTrendingMemories() {
  ...
  if (Date.now() < trendingCache.expiresAt) {
    const cached = Array.isArray(trendingCache.items)
      ? trendingCache.items
      : [];
    if (cached.length === 0) {
      container.innerHTML = `
    ...
    container.innerHTML = cached
      .map(
        (m) => `
      <div class="memory-card trending-card">
        <div class="memory-title">
          ${escapeHtml(m.title)}
          ${
            m.is_important
              ? '<span class="important-flag">Important</span>'
              : m.isFavorite
              ? '<span class="favorite-flag">Starred</span>'
              : ""
          }
        </div>
        ...
        <div class="memory-meta">
          <div class="meta-left">
            ...
            <span class="vault-label-${normalizeVaultKey(
              m.vault_type,
            )}">${getVaultLabel(m.vault_type)} Vault</span>
          </div>
          ${
            m.contributor
              ? `<span class="contributor-pill">By ${escapeHtml(
                  m.contributor,
                )}</span>`
              : ""
          }
  ...
  fetch(`${API_BASE_URL}/memories/trending?limit=12`, {
    method: "GET",
    headers: { "Content-Type": "application/json" },
    signal: buildRequestSignal(10000),
  })
    .then((response) =>
      parseJsonResponse(response, "trending-memories").then((data) => ({
        ok: response.ok,
        data,
      })),
    )
...
function getVaultLabel(vaultType) {
  const key =
    normalizeVaultKey(vaultType) || String(vaultType || "").toLowerCase();
  const labels = {
    personal: "Personal",
    learning: "Academic",
    cultural: "Cultural",
...
function renderVaultMemories(vault) {
  ...
  const vaultMemories = memories.filter(
    (m) => normalizeVaultKey(m.vault_type) === vaultKey,
  );
  ...
  const sorted = [...vaultMemories].sort(
  ...
  container.innerHTML = sorted
    ...
    <div class="memory-card">
      ...
      <div class="memory-meta">
        <span class="memory-date">${formatDate(m.timestamp)}</span>
        <button type="button" class="delete-btn" onclick="deleteMemory('${getMemoryId(
          m,
...
function renderDiaryEntries() {
  ...
  const entries = memories.filter(
    (m) => normalizeVaultKey(m.vault_type) === "personal",
  );
...
function showMessage(text, type) {
  ...
  const inlineTarget = formPageActive
...
function openAuth(mode) {
  if (!IS_AUTH_ROUTE) {
    const target =
      mode === "signup" ? "auth.html?mode=signup" : "auth.html?mode=signin";
  ...
  const resetPasswordGroup = document.getElementById(
    "auth-reset-password-group",
  );
  const resetConfirmGroup = document.getElementById("auth-reset-confirm-group");
  const passwordFormGroup = document
  ...
  if (mode === "signin") {
    ...
    if (identifierInput)
      identifierInput.placeholder = "your@email.com or username";
    if (identifierInput) identifierInput.readOnly = false;
    if (visualTitle) visualTitle.textContent = "Welcome Back";
    if (visualCopy)
      visualCopy.textContent =
        "Enter your personal details to use all Memory Vault features.";
    if (visualCta) {
      visualCta.textContent = "Create Account";
      visualCta.onclick = function () {
        openAuth("signup");
      };
    }
  } else {
    if (mode === "signup") {
      ...
      if (visualCopy)
        visualCopy.textContent =
          "Create your account and start storing reflections, notes, and wisdom.";
      if (visualCta) {
      ...
      if (visualCopy)
        visualCopy.textContent =
          "Create a new strong password for your Memory Vault account.";
      if (visualCta) {
        visualCta.textContent = "Sign In";
        visualCta.onclick = function () {
          openAuth("signin");
...
function updateSocialAuthButtons() {
  ...
  const buttons = document.querySelectorAll(
    "#auth-social-grid .auth-social-btn[data-provider]",
  );
  let enabledCount = 0;
  buttons.forEach((btn) => {
    ...
    btn.title = availability.enabled
      ? `Continue with ${label}`
      : availability.reason;
  });
...
async function startForgotPasswordFlow() {
  ...
  try {
    ...
    msgDiv.innerHTML = `<div class="message success">${escapeHtml(
      data.message || "If this email exists, a reset link has been sent.",
    )}</div>`;
  } catch (error) {
    msgDiv.innerHTML = `<div class="message error">${escapeHtml(
      error.message || "Unable to send reset link right now.",
    )}</div>`;
  }
}
    ...
    const pollTimer = setInterval(() => {
      ...
      try {
        ...
        if (!accessToken && !authCode) {
          reject(
            new Error("No auth code or access token returned by provider."),
          );
          return;
        }
...
function getSocialAuthFriendlyError(provider, rawError) {
  ...
  if (lower.includes("state mismatch"))
    return "Security validation failed. Please try sign-in again.";
...
async function socialAuthSignIn(provider) {
  ...
  if (!availability.enabled) {
    ...
    msgDivDisabled.innerHTML = `<div class="message error">${escapeHtml(
      availability.reason,
    )}</div>`;
    return;
  }
  ...
  const msgDiv = document.getElementById("auth-message");
  msgDiv.innerHTML =
    '<div class="message info">Connecting to provider...</div>';
...
if (authFormEl) {
  authFormEl.addEventListener("submit", async (e) => {
    ...
    if (!identifier) {
      msgDiv.innerHTML =
        '<div class="message error">Please enter your email or username.</div>';
      return;
    }
    ...
    if (authMode === "signup") {
      ...
      if (!username) {
        const base = identifier
          ...
          .replace(/^_+|_+$/g, "");
        username = (
          base.length >= 3 ? base : `user_${Date.now().toString().slice(-6)}`
        ).slice(0, 20);
      }
      if (!/^[a-z0-9_]{3,20}$/.test(username)) {
        msgDiv.innerHTML =
          '<div class="message error">Username must be 3-20 chars with letters, numbers, or underscore.</div>';
        return;
      }
      if (!(/[A-Za-z]/.test(password) && /\d/.test(password))) {
        msgDiv.innerHTML =
          '<div class="message error">Password must include both letters and numbers.</div>';
        return;
      }
    }
    if (authMode === "reset") {
      ...
      if (!(/[A-Za-z]/.test(newPassword) && /\d/.test(newPassword))) {
        msgDiv.innerHTML =
          '<div class="message error">New password must include letters and numbers.</div>';
        return;
      }
    ...
    try {
      ...
      msgDiv.innerHTML = `<div class="message error">${
...
function updateAuthUI() {
  ...
  if (isAuthenticated) {
    ...
    if (mobilePanelAuth)
      mobilePanelAuth.style.setProperty("display", "none", "important");
    if (mobilePanelUser)
      mobilePanelUser.style.setProperty("display", "grid", "important");

    const displayName = document.getElementById("user-display-name");
    if (displayName)
      displayName.textContent = `${getGreetingByTime()}, ${userName}`;
    ...
    if (mobileName)
      mobileName.textContent = `${getGreetingByTime()}, ${userName}`;
    ...
    if (mobilePanelAuth)
      mobilePanelAuth.style.setProperty("display", "grid", "important");
    if (mobilePanelUser)
      mobilePanelUser.style.setProperty("display", "none", "important");
  }
...
function openProfileModal() {
  ...
  // Get username from localStorage if available, otherwise it might be empty
...
async function handleProfileUpdate(event) {
  ...
  try {
    const response = await apiCall("/user/profile", "PATCH", {
      name,
      username,
    });
    if (response.success) {
      ...
      updateAuthUI();
      msgDiv.innerHTML = '<div class="message success">Profile updated!</div>';
...
async function logout() {
  ...
  if (shouldLogout) {
    ...
    msgDiv.style.cssText =
      "position: fixed; top: 6rem; right: 2rem; z-index: 200; animation: slideIn 0.3s ease;";
...
function buildAiMessage(role, bodyHtml, opts = {}) {
  ...
  turn.className = `ai-turn ${isUser ? "ai-turn-user" : "ai-turn-assistant"}${
...
function getAiMode() {
  const { mode } = getAiEls();
  const raw = String(
    mode?.value || localStorage.getItem("ai_mode") || "general",
  )
    .trim()
    .toLowerCase();
...
async function sendAiChatMessage(message, mode) {
  ...
  const call = async (withAuth) => {
    ...
    if (!response.ok || !data?.success) {
      throw new Error(
        data?.message || `AI request failed (${response.status})`,
      );
    }
...
if (aiModeEl) {
  const saved = String(
    localStorage.getItem("ai_mode") || "general",
  ).toLowerCase();
  ...
  aiFormEl.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const replyText = String(
        data.response || "I am here to help. Ask me anything.",
      );
      const typingRow = document.getElementById("ai-typing-row");
      if (typingRow) typingRow.remove();

      const assistantTurn = buildAiMessage(
        "assistant",
        formatAiResponseHtml(replyText),
      );
      ...
      const typingRow = document.getElementById("ai-typing-row");
      if (typingRow) typingRow.remove();
      const errText = `Error: ${escapeHtml(
        error?.message || "Could not get a response right now. Try again.",
      )}`;
...
function applyMemoryFilter(query) {
  ...
  if (filtered.length === 0 && normalizedQuery) {
    searchResults.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: #999; padding: 2rem;">No memories found matching "${escapeHtml(
      query,
    )}"</p>`;
...
function createMemoryCard(memory) {
  ...
  card.innerHTML = `
    <div class="memory-card-content">
      ...
      <p>${escapeHtml(memory.content.substring(0, 100))}${
    memory.content.length > 100 ? "..." : ""
  }</p>
      <div class="memory-meta">
        <span class="vault-badge" data-vault="${memory.vault_type}">${
    memory.vault_type
  }</span>
        <span class="memory-date">${new Date(
          memory.timestamp,
...
function showStats() {
  ...
  const stats = {
    ...
    learning: memories.filter(
      (m) => String(m.vault_type || "").toLowerCase() === "learning",
    ).length,
    cultural: memories.filter(
      (m) => String(m.vault_type || "").toLowerCase() === "cultural",
    ).length,
    future: memories.filter(
      (m) => String(m.vault_type || "").toLowerCase() === "future",
    ).length,
  };
...
function showFavorites() {
  ...
  if (favorites.length === 0) {
    favoritesList.innerHTML =
      '<li class="favorites-empty-message">No favorites yet. Star a memory to add it here!</li>';
  } else {
    favoritesList.innerHTML = favorites
      ...
      <li class="favorite-item">
        ...
        <div class="favorite-meta">
          <span>${m.vault_type}</span> â€¢ <span>${new Date(
          m.timestamp,
        ).toLocaleDateString()}</span>
        </div>
      </li>
...
function toggleFavorite(memoryId) {
  ...
  if (memory) {
    ...
    if (authToken) {
      // Update on backend
      apiCall(`/memories/${getMemoryId(memory)}`, "PATCH", {
    ...
    showMessage(
      memory.isFavorite ? "Added to favorites." : "Removed from favorites.",
      "success",
    );
  }
}
...
async function saveMemoriesToBackend() {
  ...
  if (!serverConnected) {
    console.warn(
      "Server not connected - memories will sync when connection is restored",
    );
    return;
  }
  ...
  try {
    for (const memory of memories) {
      await apiCall(`/memories/${memory.id}`, "PATCH", {
        isFavorite: memory.isFavorite,
      });
    }
...
function exportMemories() {
  ...
  if (modal) {
    const allOption = document.querySelector(
      'input[name="export-scope"][value="all"]',
    );
...
function updateExportPreview() {
  ...
  const exportableAll = memories.filter(
    (m) => String(m.vault_type || "").toLowerCase() !== "personal",
  );
  const importantCount = exportableAll.filter((m) =>
    Boolean(m.is_important),
  ).length;
  const selectedCount =
    scope === "important" ? importantCount : exportableAll.length;
  const suffix =
    scope === "important" ? "important memories only" : "all memories";
  ...
  if (selectedCount === 0) {
    ...
    summaryEl.textContent = `Ready to export ${selectedCount} ${
...
function getExportableMemories(scope = "all") {
  const nonPersonal = memories.filter(
    (memory) => String(memory.vault_type || "").toLowerCase() !== "personal",
  );
  const source =
    scope === "important"
